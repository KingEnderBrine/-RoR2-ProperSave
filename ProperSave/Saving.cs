...
namespace ProperSave
{
    internal static class Saving
    {
        ...
        {
            var metadata = SaveFileMetadata.GetCurrentLobbySaveMetadata() ?? new SaveFileMetadata();
            metadata.FillMetadataForCurrentLobby();

            try
            {
                using (var fileStream = ProperSavePlugin.SavesFileSystem.OpenFile(metadata.FilePath.Value, System.IO.FileMode.Create, System.IO.FileAccess.Write))
                using (var writer = new BinaryWriter(fileStream))
                {
                    metadata.Write();
                }
                ProperSavePlugin.CurrentSave = metadata;
                SaveFileMetadata.AddIfNotExists(metadata);
